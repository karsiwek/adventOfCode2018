...
fun getLaziestOne(input: String): Any {
    var schedule = input
            .split("\n")
    ...
    return guardsSleeping.entries.sortedBy { -it.value.size }.map { entry -> Pair(entry.toPair().first, entry.toPair().second.groupBy { it }.entries.sortedBy { -it.value.size }.get(0)) }.get(0);
}

fun getMinutesBetween(lastFallAsleep: LocalDateTime?, lastAwake: LocalDateTime): List<Int> {
    return (lastFallAsleep!!.minute..lastAwake.minute - 1).toList();
}


fun main(args: Array<String>) {
    System.out.print(getLaziestOne(input2));
}